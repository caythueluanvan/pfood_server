...
module.exports = (router) => {
    ...
    router.get('/products/:city/:shop/:type/:catalog/:limit/:offset', async (req, res) => {
        let sql = 'select i.ItemName, s.SourceOfItemsID, s.ItemID, s.Summary, i.ItemImage as Image, i.description as Description, s.Price, s.StartTime, s.EndTime, s.FeeID, s.view, (select prm.Promotiontypeid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typeid, (select prmt.PromotiontypeName from promotion prm, promotiontype prmt where prm.Promotiontypeid = prmt.PromotionTypeID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typename, (select prm.Promotionconditionid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionid, (select prmc.ConditionName from promotion prm, promotioncondition prmc where prm.Promotionconditionid = prmc.ConditionID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionname from sourceofitems s, items i, partner p  where s.ItemID = i.ItemID and i.PartnerID = p.PartnerID  and s.EndTime >= now() and s.StartTime <= now() and p.statusID = 1 and i.StatusID = 1'
    ...
    router.post('/products/search', async (req, res) => {
        sql = 'select i.ItemName, s.SourceOfItemsID, s.ItemID, s.Summary, i.ItemImage as Image, i.description as Description, s.Price, s.StartTime, s.EndTime, s.FeeID, s.view, (select prm.Promotiontypeid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typeid, (select prmt.PromotiontypeName from promotion prm, promotiontype prmt where prm.Promotiontypeid = prmt.PromotionTypeID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typename, (select prm.Promotionconditionid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionid, (select prmc.ConditionName from promotion prm, promotioncondition prmc where prm.Promotionconditionid = prmc.ConditionID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionname from sourceofitems s, items i, partner p  where s.ItemID = i.ItemID and i.PartnerID = p.PartnerID  and s.EndTime >= now() and s.StartTime <= now() and p.statusID = 1 and i.StatusID = 1 and (p.PartnerName like "%'+req.body.SearchText+'%" or i.ItemName like "%' +req.body.SearchText+ '%") and p.CItyID = "' +req.body.CityID+ '"'
        let rs = await dbs.execute(sql);
    ...
    router.get('/products/:SourceOfItemsID', async (req, res) => {
        sql = 'select i.ItemName, s.SourceOfItemsID, s.ItemID, s.Summary, i.ItemImage as Image, i.description as Description, s.Price, s.StartTime, s.EndTime, s.FeeID, s.view, p.*, (select prm.Promotiontypeid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typeid, (select prmt.PromotiontypeName from promotion prm, promotiontype prmt where prm.Promotiontypeid = prmt.PromotionTypeID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typename, (select prm.Promotionconditionid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionid, (select prmc.ConditionName from promotion prm, promotioncondition prmc where prm.Promotionconditionid = prmc.ConditionID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionname from sourceofitems s, items i, partner p  where s.ItemID = i.ItemID and i.PartnerID = p.PartnerID and s.SourceOfItemsID = "' + req.params.SourceOfItemsID + '"'
    ...
    router.get('/cart/:CustomerID', async (req, res) => {
        ...
        let sql = 'select i.ItemName, c.amount, s.SourceOfItemsID, s.ItemID, s.Summary, i.ItemImage as Image, i.description as Description, s.Price, s.StartTime, s.EndTime, s.FeeID, s.view from cart c, sourceofitems s, items i, partner p  where c.SourceOfItemsID = s.SourceOfItemsID and c.CustomerID = "'+req.params.CustomerID+'" and s.ItemID = i.ItemID and i.PartnerID = p.PartnerID  and s.EndTime >= now() and s.StartTime <= now() and p.statusID = 1 and i.StatusID = 1 '
        let rs2 = await dbs.execute(sql);
        if(rs2.length > 0){
        let sql1 = 'select p.*, (select prm.Promotiontypeid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typeid, (select prmt.PromotiontypeName from promotion prm, promotiontype prmt where prm.Promotiontypeid = prmt.PromotionTypeID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as typename, (select prm.Promotionconditionid from promotion prm where prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionid, (select prmc.ConditionName from promotion prm, promotioncondition prmc where prm.Promotionconditionid = prmc.ConditionID and prm.partnerID = p.partnerID and prm.StartTime <= now() and prm.EndTime >= now()) as conditionname from partner p  where p.PartnerID in (select distinct p.PartnerID from cart c, sourceofitems s, items i, partner p  where c.SourceOfItemsID = s.SourceOfItemsID and c.CustomerID = "'+req.params.CustomerID+'" and s.ItemID = i.ItemID and i.PartnerID = p.PartnerID and s.EndTime >= now() and s.StartTime <= now() and p.statusID = 1 and i.StatusID = 1 )'
        let rs1 = await dbs.execute(sql1);
        let rs = {}
        rs.Partner = rs1[0]
        rs.ListItems = rs2
