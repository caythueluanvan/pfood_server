...
router.get('/recommend/:customerid', async function (req, res) {
    ...
    if (userToRecommend.length) {
        ...
        rsRecommend = await dbs.execute(`select it.ItemName, s.Summary, s.Price, s.SourceOfItemsID, i.id, s.ItemID, i.ItemImage Image, i.Description  from sourceofitems s, itempartner i, items it  where s.ItemID = i.id and i.itemid = it.ItemID and s.EndTime >= now() limit 5  `, []);
    }
