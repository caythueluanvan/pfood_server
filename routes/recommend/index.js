...
router.get('/recommend/:customerid', async function (req, res) {
    ...
    if (userToRecommend.length) {
        ...
        rsRecommend = await dbs.execute(`select i.ItemName, s.Summary, s.Price, s.SourceOfItemsID, s.ItemID, s.Image, s.Description  from sourceofitems s, items i  where s.ItemID = i.ItemID and i.ItemID in ( ? ) and s.EndTime >= now() limit 5 `, [bind.toString()]);
    } else {
        rsRecommend = await dbs.execute(`select i.ItemName, s.Summary, s.Price, s.SourceOfItemsID, s.ItemID, s.Image, s.Description  from sourceofitems s, items i  where s.ItemID = i.ItemID and s.EndTime >= now() limit 5 `, []);
    }
