...
const genSourceOfItem = async () => {
    try {
        const rsSchedule = await dbs.execute('select s.item_id, s.timefrom, s.timeto, s.price, s.amount, i.itemimage, i.description from scheduleitem s, items i where dayofweek =? and i.itemid = s.item_id and i.statusid = 1', [new Date().getDay()])
        ...
        async.forEachOf(rsSchedule, async (value, key) => {
            ...
            bind.push([id, value.item_id, value.amount, value.itemimage, value.price, startTime, endTime, value.description])
        }, async err => {
              if (err) console.log(err);
              await dbs.execute(`insert into sourceofitems(sourceofitemsid, itemid, summary, image, price, starttime, endtime, description) values ?`, [bind]);
        });
